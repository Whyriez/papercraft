...
const BannerSlider = ({ banners }: { banners: any[] }) => {
    ...
    return (
        <div className="relative h-[400px] w-full overflow-hidden rounded-[36px] border border-gray-800 shadow-lg shadow-black/20 sm:h-[480px] lg:h-[540px]">
            {slides.map((slide, index) => {
                ...
                const rawImgPath = isCustom
                    ? slide.image_path
                    : (slide.papercraft?.primaryImage?.image_path ??
                      slide.papercraft?.primary_image?.image_path);
                const linkDest = isCustom
                    ? slide.link_url
                    : slide.papercraft
                ...
                return (
                    <div
                        key={slide.id}
                        className={`absolute inset-0 transition-all duration-1000 ease-in-out ${isActive ? 'z-10 scale-100 opacity-100' : 'z-0 scale-105 opacity-0'}`}
                    >
                        <div className="absolute inset-0 bg-gray-900">
                            {imgPath ? (
                                ...
                                <div className="h-full w-full bg-gray-900 bg-[radial-gradient(circle_at_top_right,rgba(75,85,99,0.4),transparent_50%),radial-gradient(circle_at_bottom_left,rgba(55,65,81,0.4),transparent_50%)]"></div>
                            )}
                        </div>

                        <div className="absolute inset-0 bg-gradient-to-t from-gray-950/90 via-gray-950/30 to-transparent" />

                        <div className="absolute right-0 bottom-0 left-0 p-8 text-white sm:p-12 lg:p-16">
                            <div
                                className={`transform transition-all delay-200 duration-700 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
                            >
            ...
            {slides.length > 1 && (
                <div className="absolute right-8 bottom-8 z-20 flex gap-2 lg:right-12 lg:bottom-12">
                    {slides.map((_, idx) => (
                        <button
                            key={idx}
...
const CategoryCard = ({
    ...
    return (
        <Link
            ...
            className="group relative flex min-h-[160px] flex-col justify-between overflow-hidden rounded-[28px] border border-gray-800 bg-gray-900 p-6 shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
        >
            ...
            {hasImage ? (
                <>
                    <img
                        ...
                        alt={category.name}
                        className="absolute inset-0 h-full w-full rounded-[28px] object-cover transition-transform duration-700 group-hover:scale-110"
                    />
                    <div className="absolute inset-0 rounded-[28px] bg-gradient-to-t from-gray-950/90 via-gray-950/40 to-gray-950/10" />
                </>
            ) : (
                <div className="absolute inset-0 rounded-[28px] bg-gray-900" />
            )}
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
                <div className="flex items-start justify-between gap-4">
                    ...
                    {hasChildren && (
                        <div
                            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-sm font-black shadow-sm sm:h-12 sm:w-12 ${hasImage ? 'border border-white/20 bg-black/30 text-white backdrop-blur-md' : 'border border-gray-700 bg-gray-800 text-gray-300'}`}
                        >
                ...
                <div
                    className={`mt-6 flex items-center justify-between border-t pt-4 text-sm font-semibold ${hasImage ? 'border-white/20 text-gray-200' : 'border-gray-800 text-gray-400'}`}
                >
                    <span>
                        {hasChildren
                            ? 'View Sub-Categories'
    ...
    return (
        ...
        >
            ...
            <div className="relative rounded-[30px] bg-gray-900">
                <div className="relative aspect-4/3 overflow-hidden rounded-t-[30px] border-b border-gray-800 bg-gray-800">
                    {coverImage ? (
                        <img
                ...
                <div className="relative flex flex-1 flex-col gap-4 p-6 pt-5">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <p className="text-[11px] font-bold tracking-[0.28em] text-gray-400 uppercase">
                                {item.category?.name}
                            </p>
                            <h3 className="mt-2 line-clamp-2 text-lg leading-snug font-extrabold text-gray-100 transition-colors group-hover:text-white">
                                {item.title}
                            </h3>
                        </div>
                    </div>
...
export default function Home({
    ...
    return (
        ...
        >
            ...
            <main
                id="home"
                className="relative mx-auto max-w-7xl px-4 pt-28 pb-20 sm:px-6 lg:pt-32"
            >
                {/* 🌟 BANNER SLIDER 🌟 */}
                <section>
                ...
                <section className="mt-8 flex flex-wrap justify-center gap-3">
                    ...
                    {quickCategories.map((category) => (
                        <Link
                            ...
                            className={`rounded-full border px-5 py-2.5 text-sm font-bold shadow-sm transition-all ${filters.category === category.slug ? 'border-gray-500 bg-gray-800 text-white' : 'border-gray-800 bg-gray-900 text-gray-400 hover:-translate-y-0.5 hover:bg-gray-800 hover:text-gray-200'}`}
                        >
                            {category.name}
                        </Link>
                    ))}
                                        ...
                                        {papercrafts?.links &&
                                            papercrafts.links.length > 3 && (
                                                <div className="mt-12 flex flex-wrap items-center justify-center gap-2">
                                                    {papercrafts.links.map(
                                                        (link, key) => (
                                                            <Link
                                                                key={key}
                                                                href={
                                                                    link.url
                                                                        ? `${link.url}#filtered-view`
                                                                        : '#'
                                                                }
                ...
                <footer
                    className="mt-24 rounded-[34px] border border-gray-800 bg-gray-900 px-6 py-12 shadow-lg sm:px-10 lg:py-16"
                    id="contact"
                >
